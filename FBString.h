...
// http://stackoverflow.com/a/24094278
namespace std _GLIBCXX_VISIBILITY(default) {
_GLIBCXX_BEGIN_NAMESPACE_VERSION // http://stackoverflow.com/a/10470920
#else
namespace folly {
#endif
...
inline void pod_fill(Pod* b, Pod* e, T c) {
  ...
  /*static*/ if (sizeof(T) == 1) {
    memset(b, c, e - b); // if T is of size 1, i.e char , just memset it
  } else {  // else
    /*
     *  ~7u =>
     *  00000111 => 11111000 then & with e-b
     *  i.e, ignore if e-b's distance is <= 7
     *  we only care about > 7 distance
...
VS_Study: it is the store_ datamember of fbstring
...
# pragma GCC diagnostic push
# pragma GCC diagnostic ignored "-Wuninitialized" // why ignore??
#endif
 ...
 * most significant bits of the rightmost char of the storage.
    就是 MediumLarge structure的 capacity_ data member!
    這capacity_ 就是最右邊的 storage of MediumLarge Strucutre
     If
 ...
 *
 * VS_Study:
 * 1. small str :[0-23] char : 8 * 23 + 8(store size) : store on stack
 * 2. medium str : [24 - 254] char : [254 - 8*3+1] : store on malloc-mem
 * 3. large str : [255 - unlimit] : [ulimit - 255] : same as medium, but ref counted
 *      copy lazily . Ref count store before char array. i.e MSByte of string
 *
 * how to differentiate from these 3 types of string?
 * xx000000 <= the LSByte of a string. xx is the flag
 * 1. xx is unset : small str , and it's size is stored in 000000
 * 2. xx is set to 1x : medium str.
 * 3. xx is set to x1 : large str.
 */
...
public:
  ...
  /*
    ...
    // sizeof this struct is 8 + 8 + 8 = 24 bytes on 64bit machine
  struct MediumLarge {
    Char * data_;
    size_t size_;
    // capacity 含 大小 與 catagory info
    size_t capacity_;
  ...
  enum {
    lastChar = sizeof(MediumLarge) - 1, // 23 bytes
    maxSmallSize = lastChar / sizeof(Char),  // 23 bytes
...
/*
 *  Default constructor / small string
 */
  fbstring_core() noexcept {
    // Only initialize the tag, will set the MSBs (i.e. the small
    // string size) to zero too
    // capacity_ is 8 bytes
    // thus, 23 is on the MSBytes, the rest 7 bytes are 0
    // 00010111  + 7 bytes of 0
    // since xx => 00, thus default small size string
  ...
  fbstring_core(const Char *const data, const size_t size)
    ...
    if (size <= maxSmallSize) {
      ...
      ml_.data_ = newRC->data_; // assigning address of newRc.data_ to ml.data_ ptr.
      ml_.size_ = size; // does not contains \0
  ...
  ~fbstring_core() noexcept {
    ...
    RefCounted::decrementRefs(ml_.data_); // while == 1, free mem.
  }
    ...
    if (category() == Category::isLarge) {
      ...
      if (RefCounted::refs(ml_.data_) > 1) {
        ...
        if (minCapacity > ml_.capacity()) {
          ...
          auto const newRC =
               RefCounted::reallocate(ml_.data_, ml_.size_, // use reallocate here!!
  ...
  void writeTerminator() {
    if (category() == Category::isSmall) {
      const auto s = smallSize(); // 23 - 23 = 0
      if (s != maxSmallSize) {  //already has \0
        small_[s] = '\0'; // littl endian 23 bytes + 1 byte 最右邊的1 byte 設為 \0
      }
    } else {
...
private:
  ...
  enum {
    ...
    maxSmallSize = lastChar / sizeof(Char), // 23
  ...
  size_t smallSize() const {
    ...
    return static_cast<size_t>(maxSmallSize) // 23
      - static_cast<size_t>(small_[maxSmallSize]); // small_ is sizeof 24 , small_[23]
  }
